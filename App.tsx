...
import {
  ...
  Menu, Settings, History, Layout, ChevronRight, Sparkles, Grid,
  ChevronLeft, MousePointer2, Users
} from 'lucide-react';
...
export default function App() {
  ...
  const [settings, setSettings] = useState(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : { autoFullscreen: false, enableTabs: false };
    } catch {
  ...
  const similarGames = useMemo(() => {
    ...
    return games
      ...
      .map((g) => ({
        game: g,
        score: g.Categories.filter((c) => selectedSet.has(c)).length,
      }))
  ...
  const playerLeftGames = useMemo(() => {
    const fromSearch = games.filter((game) => game.Title.toLowerCase().includes(playerSearch.toLowerCase()));
    if (playerSearch.trim()) return fromSearch;
    return similarGames;
  ...
  const handleGameSelect = useCallback((game: Game) => {
    ...
    if (settings.enableTabs) {
      setOpenGames((prev) => (prev.find((g) => g.Title === game.Title) ? prev : [...prev, game]));
    } else {
      setOpenGames([game]);
    }
  ...
  const handleWheelGuard: WheelEventHandler<HTMLDivElement> = (event) => {
    event.preventDefault();
  };

  const updateLogs = [{ version: '2.1', date: '2026-03-09', sections: [{ title: 'Coral', changes: ['Performance pass', 'Player sidebar recommendations', 'Custom cursor + right click commands', 'Social page (WIP)'] }] }];
    ...
    >
      <header className={`sticky top-0 z-40 glass px-6 py-4 mb-8 transition-all duration-300 ${isPlayerOpen ? 'opacity-0 pointer-events-none -translate-y-full' : 'opacity-100 translate-y-0'}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <h1 className="text-2xl font-bold tracking-tight text-white leading-none">Coral <span className="text-sm text-[var(--fg-muted)]">Scientific Calculator</span></h1>
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden md:block group">
              ...
              <input type="text" placeholder="Search games..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-[var(--bg-surface)] border-none rounded-2xl py-2.5 pl-11 pr-4 text-sm focus:ring-2 focus:ring-[var(--accent)] w-64 transition-all placeholder:text-[var(--fg-muted)]/50 text-[var(--fg)]" />
            </div>

            <button onClick={() => setShowCategoryPicker((prev) => !prev)} className={`p-3 rounded-2xl transition-all shadow-sm ${selectedCategories.length ? 'bg-[var(--accent)] text-white' : 'bg-[var(--bg-surface)] hover:bg-[var(--bg-card)] text-[var(--fg)]'}`}>
        ...
        <div className="md:hidden mt-4 space-y-3">
          ...
          <button onClick={() => setShowCategoryPicker((prev) => !prev)} className={`w-full p-3 rounded-2xl text-sm font-semibold ${selectedCategories.length ? 'bg-[var(--accent)] text-white' : 'bg-[var(--bg-surface)] text-[var(--fg)]'}`}>Multi category filter</button>
        </div>

        {showCategoryPicker && (
          <div className="max-w-7xl mx-auto mt-4 bg-[var(--bg-surface)] rounded-2xl p-4 border border-white/10">
            <div className="flex flex-wrap gap-2 mb-3">
              {categories.map((cat) => (
                <button key={cat} onClick={() => toggleCategory(cat)} className={`px-3 py-1.5 rounded-xl text-xs ${selectedCategories.includes(cat) ? 'bg-[var(--accent)] text-white' : 'bg-[var(--bg-card)] text-[var(--fg-muted)]'}`}>{cat}</button>
              ))}
            </div>
            <div className="flex gap-2">
              <button onClick={() => setShowCategoryPicker(false)} className="px-3 py-1.5 rounded-xl bg-[var(--accent)] text-white text-xs">Filter</button>
              <button onClick={() => setSelectedCategories([])} className="px-3 py-1.5 rounded-xl bg-[var(--bg-card)] text-[var(--fg-muted)] text-xs">Clear</button>
            </div>
          </div>
        )}
      </header>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }} className="fixed right-0 top-0 bottom-0 w-80 bg-[var(--bg-surface)] border-l border-white/5 z-[70] shadow-2xl flex flex-col rounded-l-3xl overflow-hidden">
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-[var(--bg-card)]">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-white">{menuView === 'settings' ? 'Settings' : menuView === 'tabs' ? 'Tabs' : 'Menu'}</h2>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {menuView === 'main' ? (
                  <div className="space-y-3">
                    ...
                    {settings.enableTabs && <button onClick={() => setMenuView('tabs')} className="w-full flex items-center gap-4 p-4 rounded-2xl bg-[var(--bg-card)]"><Layout className="w-5 h-5" />Tabs<ChevronRight className="w-4 h-4 ml-auto" /></button>}
                  ...
                  <div className="space-y-3">
                    {openGames.map((game) => (
                      <div key={game.Title} className="flex items-center justify-between p-3 rounded-2xl bg-[var(--bg-card)]">
                        <button onClick={() => { setSelectedGame(game); setIsPlayerOpen(true); }} className="flex-1 text-left truncate">{game.Title}</button>
                        <button onClick={() => closeTab(game.Title)} className="p-2"><X className="w-4 h-4" /></button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="p-6 bg-[var(--bg-card)] text-center"><p className="text-xs text-[var(--fg-muted)] font-medium">Coral • Scientific Calculator</p></div>
            </motion.div>
          </>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showUpdateLog && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[var(--bg)] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 max-w-4xl mx-auto w-full">{updateLogs.map((log) => <div key={log.version}><h3 className="text-3xl font-bold text-[var(--accent)] mb-4">v{log.version}</h3>{log.sections.map((section) => <div key={section.title} className="bg-[var(--bg-surface)] rounded-3xl p-6 mb-4"><h4 className="font-bold mb-3">{section.title}</h4>{section.changes?.map((c) => <p key={c} className="text-sm text-[var(--fg-muted)]">• {c}</p>)}</div>)}</div>)}</div>
          </motion.div>
        )}
      </AnimatePresence>
